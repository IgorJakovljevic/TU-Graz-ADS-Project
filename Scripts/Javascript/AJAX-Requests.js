var AjaxRequests = {
        ...
        xmlhttp.onreadystatechange = function () {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                if (!xmlhttp.responseText) {
                    document.getElementById("content").innerHTML = "<h1>Username or password didn't match</h1>";
                    return;
                }
                ...
                document.getElementById('navigation').removeChild(document.getElementById("login"));
    ...
    deleteFile: function (id) {
        document.getElementById('content').removeChild(document.getElementById('file-' + id));
        var xmlhttp;
        if (window.XMLHttpRequest) { // code for IE7+, Firefox, Chrome, Opera, Safari
            xmlhttp = new XMLHttpRequest();
        } else { // code for IE6, IE5
